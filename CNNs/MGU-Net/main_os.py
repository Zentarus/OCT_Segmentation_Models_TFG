...
def train(args, train_loader, model, criterion2, optimizer, epoch, print_freq=10):
    ...
    losses = AverageMeter()
    dice = AverageMeter()
    dice_meters = [AverageMeter() for _ in range(1, args.num_classes)]  # Una por clase, ignorando fondo
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        ret_d = compute_dice(label_seg, pred_seg, args.num_classes)  # Modificado para pasar el numero de clases a segmentar
        dice_score = compute_single_avg_score(ret_d, args.num_classes) # Modificado para pasar el numero de clases a segmentar
        # update dice score
...
def eval(phase, args, eval_data_loader, model, result_path=None, logger=None):
    ...
    for iter, (image, label, imt, imn) in enumerate(eval_data_loader):
        with torch.no_grad():
            ...
            # calculate dice and pa score 
            label_seg = label.numpy().astype('uint8')
            pred_seg_batch.append(pred_seg)
            label_seg_batch.append(label_seg)
            ret_d = compute_dice(label_seg, pred_seg, args.num_classes) # Modificado para pasar el numero de clases a segmentar
            ret_p = compute_pa(label_seg, pred_seg, args.num_classes) # Modificado para pasar el numero de clases a segmentar
            ret_dice.append(ret_d)
            ret_pa.append(ret_p)
            dice_score = compute_single_avg_score(ret_d, args.num_classes) # Modificado para pasar el numero de clases a segmentar
            mpa_score = compute_single_avg_score(ret_p, args.num_classes) # Modificado para pasar el numero de clases a segmentar
...
def append_metrics_to_json(file_path, metrics_dict):
    """Anade un nuevo registro al JSON, creandolo si no existe."""
...
def train_seg(args, train_result_path, train_loader, eval_loader):
    ...
    # load the network
    net = net_builder(args.name, n_classes=args.num_classes) # Modificado para pasar el numero de clases a segmentar
    model = torch.nn.DataParallel(net).cuda()
    print('#' * 15, args.name, '#' * 15)

    # define loss function
    criterion2 = loss_builder2(args.num_classes) # Modificado para pasar el numero de clases a segmentar
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # train for one epoch
        train_metrics = train(args, train_loader, model, criterion2, optimizer, epoch)
        loss, dice_train = train_metrics[0], train_metrics[1]
        dice_train_classes = train_metrics[2:]  # Lista de Dice por clase (sin fondo)

        # evaluate on validation set
        eval_metrics = eval('train', args, eval_loader, model)
        dice_val = eval_metrics[0]
        dice_val_classes = eval_metrics[1:-1]  # Lista de Dice por clase (sin fondo), excluye dice_list
        ...
        # Guardar metricas en JSON, para cada epoca
        metrics_entry = {
            "epoch": epoch,
...
def main():
    ...
    t = [
        dt.Label_Transform(class_mapping=class_mapping),  # Pasa el mapeo
        dt.ToTensor(),
        normalize
    ]
